...
def health():
    ...
    print('curl -X GET '+str(masterUrl)+'/api/v1/namespaces/default/pods/ --header "Authorization: Bearer '+str(TOKEN)+'" --insecure | jq . > all-pods.json')
    ...
    for value in redisResult.keys():
        ...
        for container in temp:
            ...
            for value in temp1.keys():
                returnFile.write('      state:'+str(temp1[value])+'\n')
                print(temp1[value])
